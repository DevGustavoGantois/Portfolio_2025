      ...
      {homeProjectData.map((item, index) => (
        <div key={index} className="w-full flex-1 mb-16">
          <article className="flex items-center justify-center mb-8">
            <h1 className="text-white text-center text-4xl lg:text-[40px]">{item.title}</h1>
          </article>
            ...
            {item.cards.map((project, idx) => (
              ...
              >
                <div className="flex flex-col justify-center lg:justify-start items-center gap-6 px-2 py-8">
                  <figure className="w-full bg-black/5">
                    {project.img && (
                      <Link href={project.href || "#"} target={project.href ? "_blank" : undefined}>
                      <Image
                        src={project.img}
                        width={1200}
                        height={1200}
                        alt={project.title}
                        className="w-full h-full rounded-lg transition-all duration-1000 hover:scale-[1.2]"
                      />
                      </Link>
                    )}
                  </figure>
                  ...
                  <article className="flex flex-col gap-4 justify-start w-full">
                    <div className="flex items-center justify-between">
                      <h1 className="text-white/90 text-base lg:text-xl">{project.title}</h1>
                      <h2 className="text-white/70 text-base lg:text-xl">{project.date}</h2>
                    </div>
